...
class StatsCalculator {
    ...
    addHand(hand) {
        ...
        const preflopActions = hand.actions.filter(a => a.round === 0 && a.isHero);

        const hasVoluntaryAction = preflopActions.some(a => 
        ...
        const hasRaise = preflopActions.some(a => a.type === ACTION_TYPES.RAISE);

        if (hasRaise) {
            stats.pfrHands++;
        }

        const preflopActionsBeforeHero = hand.actions.filter(a => 
            a.round === 0 && 
            !a.isHero && 
        ...
        const enemyRaise = hand.actions.some(a => 
            a.round === 0 && 
            !a.isHero && 
            (a.type === ACTION_TYPES.RAISE || a.type === ACTION_TYPES.BET)
        );
        ...
        const hasFold = hand.actions.some(a => 
            a.round === 0 && 
            a.isHero && 
        ...
        const enemyRfi = hand.actions.some(a => 
            a.round === 0 && 
            !a.isHero && 
            (a.type === ACTION_TYPES.RAISE || a.type === ACTION_TYPES.BET) &&
            hand.actions.filter(b => 
                b.round === 0 && 
                !b.isHero && 
        ...
        if (enemyRfi) {
            ...
            const hasCall = hand.actions.some(a => 
                a.round === 0 && 
                a.isHero && 
                a.type === ACTION_TYPES.CALL
            );
        ...
        const positionGroup = getPositionGroup(hand.heroPosition);
        if (stats.positions[positionGroup]) {
            const posStats = stats.positions[positionGroup];
            posStats.hands++;
        ...
        if (hand.heroCards) {
            ...
            const cardStats = stats.handsByCards[cardsKey];
            cardStats.hands++;
